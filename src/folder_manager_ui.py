...
class FolderManagerUI(Generic[T]):
        ...
        with st.expander(
            ...
            with col_unload:
                if st.button(
                    ...
                    use_container_width=True,
                    help=f"Unload this folder ({self.cfg.item_type_label}s are saved)",
                ):
            ...
            if item_count == 0:
                st.info(
                    f"No {self.cfg.item_type_label}s in this folder. "
                    f"Click 'New {self.cfg.item_type_label.title()}' to create one."
                )
            else:
    ...
    def _get_item_id(self, item: T) -> str:
        """Get the ID from an item. Assumes item has model_id or prompt_id attribute."""
        if hasattr(item, "model_id"):
            return item.model_id
        if hasattr(item, "prompt_id"):
            return item.prompt_id
        raise AttributeError(f"Item {item} has no model_id or prompt_id attribute")
    ...
    def render_all_folders(
        ...
        if len(self._loaded_folders) == 0:
            st.info(
                f"No folders loaded. Select a folder above to load "
                f"{self.cfg.item_type_label}s."
            )
        else:
